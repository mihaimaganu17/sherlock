...
impl<T> LockCell<T> {
    /// Move up a `val` into a `LockCell`, a type which allows inner mutability around ticket 
    /// spinlocks 
...
/// A guard structure which can implement `Drop` such that locks can be automatically released
/// based on scope
pub struct LockCellGuard<'a, T: ?Sized> {
    /// A reference to the value that we currently have exclusive access to
    cell: &'a LockCell<T>,
}
...
impl<'a, T: ?Sized> Deref for LockCellGuard<'a, T> {
    ...
    fn deref(&self) -> &Self::Target {
        unsafe {
            // We deref the raw pointer from `get` using * to get the type of the pointer and then
